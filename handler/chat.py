...
from handler.chat_kafka import get_kafka_consumer, get_kafka_producer
    ...
    try:
        ...
        try:
            await redis.publish(
                f"room:{room_id}",
                json.dumps(
                    {"user": current_user.name, "message": "has disconnected."}
                ),
            )
        except WebSocketDisconnect:
            pass
    ...
    try:
        ...
        try:
            ...
            await kafka_producer.send_and_wait(
                room_id, disconnect_msg.encode("utf-8")
            )
        except WebSocketDisconnect:
            pass
    finally:
